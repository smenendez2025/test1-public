...
async def login_for_access_token(request: TokenRequest):
    ...
    # Si el usuario no existe o no tiene rol, se asumirá 'user' por defecto.
    role_query = "MATCH (u:User {githubId: $user_id}) RETURN u.role AS role"
    result = graph.query(role_query, params={"user_id": user_id})
    
    user_role = "user" # Rol por defecto
    if result and result[0] and result[0].get("role"):
